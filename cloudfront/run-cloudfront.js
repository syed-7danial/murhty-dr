...
const processCloudFront = async (cloudfrontSettings) => {
    ...
    let cloudfrontIds = cloudfrontSettings.CLOUDFRONT_IDS
    try {
        cloudfrontIds = JSON.parse(cloudfrontSettings.CLOUDFRONT_IDS);
    } catch (error) {
        // Fallback to manual parsing if JSON parsing fails
        cloudfrontIds = cloudfrontSettings.CLOUDFRONT_IDS.replace(/^\[|\]$/g, '').split(',').map(ip => ip.trim());
    }
    
    let filteredCloudfrontSettings = cloudfrontSettings.cloudfront.filter(distribution =>
        cloudfrontIds.includes(distribution.id)
    );

    try 
    {
        for (let distribution of filteredCloudfrontSettings) {
            ...
            find_and_update_cloudfront_origin(defaultBehavior, cloudfrontConfig.behaviors, "path", "Default", cloudfrontSettings.switching_to)
            
            let cacheBehaviors = distributionConfig.CacheBehaviors.Items;
            for (let behavior of cacheBehaviors)
                find_and_update_cloudfront_origin(behavior, cloudfrontConfig.behaviors.slice(1), "path", behavior.PathPattern, cloudfrontSettings.switching_to)
...
const find_and_update_cloudfront_origin = function(behaviour, arr, key, val, switching_to_env) {
    ...
    let origin = null;
    if (matched_origin) {
        if (switching_to_env === awsEnvironment.ACTIVE_ENV)
            origin = matched_origin.active_origin;
        else
            origin = matched_origin.failover_origin;
...
const mainFunction = async () => {
    ...
    envs['CLOUDFRONT_IDS'] = process.env.CLOUDFRONT_IDS
