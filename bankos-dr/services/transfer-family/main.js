...
//         ...userData,
//         HomeDirectory: `/${targetBucket}/${userData.UserName}`,
//         Role: targetRole
//       };
      
//       const params = {
//         ServerId: serverId,
//         HomeDirectory: modifiedUserData.HomeDirectory,
//         HomeDirectoryType: modifiedUserData.HomeDirectoryType,
//         Role: modifiedUserData.Role,
...
//           return {
//             ...mapping,
//             Entry: mapping.Entry.replace(/\/[^\/]+\//, `/${targetBucket}/`)
//           };
//         });
//       }
...
//         if (global.DRY_RUN) {
//           custom_logging(chalk.yellow(`[DRY RUN] Would replicate users from ${failoverServerConfig.serverId} to ${activeServerConfig.serverId}`));
//           custom_logging(chalk.yellow(`[DRY RUN] Would update HomeDirectory to use bucket: ${activeServerConfig.bucket}`));
...
//         if (global.DRY_RUN) {
//           custom_logging(chalk.yellow(`[DRY RUN] Would replicate users from ${activeServerConfig.serverId} to ${failoverServerConfig.serverId}`));
//           custom_logging(chalk.yellow(`[DRY RUN] Would update HomeDirectory to use bucket: ${failoverServerConfig.bucket}`));
...
const createTransferUser = async (transferClient, serverId, userData, targetBucket, targetRole) => {
    try {
      ...
      // Handle HomeDirectory based on HomeDirectoryType
      if (userData.HomeDirectoryType === 'PATH') {
        // For PATH type, update HomeDirectory to use the target bucket
        params.HomeDirectory = `/${targetBucket}/${userData.UserName}`;
      } else if (userData.HomeDirectoryType === 'LOGICAL') {
        // For LOGICAL type, update HomeDirectoryMappings to use the target bucket
        // Do not set HomeDirectory for LOGICAL type
        if (userData.HomeDirectoryMappings && userData.HomeDirectoryMappings.length > 0) {
          params.HomeDirectoryMappings = userData.HomeDirectoryMappings.map(mapping => {
            // Replace the bucket name in the Target path
