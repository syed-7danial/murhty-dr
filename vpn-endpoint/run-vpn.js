...
const processVpnEndpoint = async (environmentConfig) => {
    ...
    for (let vpn_endpoint of environmentConfig.vpn_endpoints) {
        ...
        let ips = environmentConfig.BANK_LIST
        try {
            ips = JSON.parse(environmentConfig.BANK_LIST);
        } catch (error) {
            // Fallback to manual parsing if JSON parsing fails
            ips = environmentConfig.BANK_LIST.replace(/^\[|\]$/g, '').split(',').map(ip => ip.trim());
        }
        ...
        try {
            await modifyVpnConnectionRoute(ec2toAddIpsClient, endpointToAddIpsTo, ips, true)
        }
        catch (error) {
        ...
        if (global.PROCESS_CURRENT_ENVIRONMENT) {
            try {
                await modifyVpnConnectionRoute(ec2toRemoveIpsClient, endPointToRemoveIpsFrom, ips, false)
            }
            catch (error) {
                custom_logging(chalk.red("Error: ") + error.message);
            }
...
const mainFunction = async () => {
    ...
    const file = './vpn-endpoint/configuration.json';
    let envs = await readAndParseFile(file)
    envs['switching_to'] = process.env.SWITCHING_TO
    envs['VPN_CONFIGURATION_ID'] = process.env.VPN_CONFIGURATION_ID
    envs['BANK_LIST'] = process.env.BANK_LIST
    ...
    let endpoints = envs.vpn_endpoints.find(endpoint => endpoint.id === process.env.VPN_CONFIGURATION_ID)
    envs['vpn_endpoints'] = [endpoints]
